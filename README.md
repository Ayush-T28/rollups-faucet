...
These testnets are often the testbeds for any new native feature of the network itself, or any dApp or [Subnet](https://www.dcm.network/subnets) that is going live on the main network (mainnet). For example, [Testnet](https://docs.dcm.network/quickstart/fuji-workflow) network is the testnet for Dcomm's mainnet.

Besides Testnet, [Dcomm Faucet](https://faucet.dcm.network/) can be used to get free coins on these testnets Subnets like -
* [Subnet-test Testnet](https://faucet.dcm.network?subnet=test)

You can use this [repository](https://github.com/dcomm-tech/DComm-faucet) to deploy your faucet or just make a PR with the [configurations](https://github.com/dcomm-tech/DComm-faucet/blob/main/config.json) of the Subnet. This faucet comes with many features like multiple chain support, custom rate-limiting per Subnet, captcha verification, and concurrent transaction handling.

## TL;DR

A [Faucet](https://faucet.dcm.network/) powered by Dcomm for Test Network and other Subnets. You can -

* Request test coins for the supported Subnets
* Integrate your EVM Subnet with the faucet by making a PR with the [chain configurations](https://github.com/dcomm-tech/DComm-faucet/blob/main/config.json)
* Fork the [repository](https://github.com/dcomm-tech/DComm-faucet) to deploy your faucet for any EVM chain

## Adding a New Subnet

You can also integrate a new Subnet on the live [faucet](https://faucet.dcm.network) with just a few lines of configuration parameters. All you have to do is make a PR on the [Dcomm Faucet](https://github.com/dcomm-tech/DComm-faucet) git repository with the Subnet's information. The following parameters are required.
...
Add the configuration in the array of `evmchains` inside the [config.json](https://github.com/dcomm-tech/DComm-faucet/blob/main/config.json) file and make a PR.

## Building and Deploying a Faucet

You can also deploy and build your faucet by using the [Dcomm Faucet](https://github.com/dcomm-tech/DComm-faucet) repository.
...
```bash
git clone https://github.com/dcomm-tech/DComm-faucet
```
...
"evmchains": [
    {
            "ID": "ACT",
            "NAME": "Dcomm-testnet (Action-Chain)",
            "TOKEN": "DCM",
...
```
In the above configuration drip amount is in `nDCM` or `gwei`, whereas fees are in `wei`. For example, with the above configurations, the faucet will send `1 DCM` with maximum fees per gas being `100 nDCM` and priority fee as `2 nDCM`.
...
```bash
curl http://localhost:8000/api/faucetAddress?chain=ACT
```
...
```bash
curl http://localhost:8000/api/getBalance?chain=ACT
```
...
```bash
{
    "message": "Transaction successful on Dcomm Action Chain!",
...
So, if you want to deploy this faucet, and have some reverse proxies in between, then you should configure this inside the `GLOBAL_RL` key of the `config.json` file. If this is not configured properly, then the users might get rate-limited very frequently, since the server-side proxy's IP addresses are being viewed as the client's IP. You can verify this in the code [here](https://github.com/dcomm-tech/DComm-faucet/blob/23eb300635b64130bc9ce10d9e894f0a0b3d81ea/middlewares/rateLimiter.ts#L25).
...
Captcha is required to prove the user is a human and not a bot. For this purpose, we will use [Google's Recaptcha](https://www.google.com/recaptcha/intro/v3.html). The server side will require `CAPTCHA_SECRET` that should not be exposed. You can set the threshold score to pass the captcha test by the users [here](https://github.com/dcomm-tech/DComm-faucet/blob/23eb300635b64130bc9ce10d9e894f0a0b3d81ea/middlewares/verifyCaptcha.ts#L20).
...
### Visit Dcomm Faucet Website

Go to https://faucet.dcm.network. You will see various network parameters like network name, faucet balance, drop amount, drop limit, faucet address, etc.
...
The transaction hash that you get for each drop is pre-computed using the expected nonce, amount, and receiver's address. Though transactions on Dcomm are near-instant, the explorer may take time to index those transactions. You should wait for a few more seconds, before raising any issue or reaching out to us.